...
7. 在函数声明、函数表达式、函数调用、对象创建、数组创建、for语句等场景中，**禁止**在 , 或 ; 前换行；

8. 不同行为或逻辑的语句集，**推荐**空行隔开，更易阅读；

9. 对于 `if...else...` 、 `try...catch...finally` 等语句，**推荐**使用在 } 号后添加一个换行。
...
<!-- bad -->
<ul>
  <li v-for="todo in todos" v-if="todo.id !== 1" :key="todo.id">{{ todo.text }}</li>
</ul>
```
